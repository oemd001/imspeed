class Solution {
    ...
    private String helper(String s, int numRows) {
        ...
        Another theme we'll need to identify: the letters in between
            ...
            Basically, what we do this is complete one cycle. After we complete one cycle, we backtrack back to 
account for the value in the middle
            e.g.numRows = 3: on row 2, we have 1 (A)
                we add 4. We get our "L". However, we backtrack by 2 to get our "P"
                This formula we can use to find our backtrack values is: n - 2i (where n is our number of steps 
and i is the interval where the backtracking begins)
        */
        ...
        for (int i = 0; i < numRows; i++) { //this part gives us the number of rows we are going to utilize here
            ...
            for (int j = i; j < length; j += n) {
                ...
                count++;
                if (stp > 0 && stp < n && j + stp < length) { //this part gives us the middle characters. 
remember, the range must be within 0 to n
                    c[count] = s.charAt(j + stp);             //in addition, the stp > 0 and stp < n part just 
ensures that we are not interferring with the "column areas" when we want to add the middle values
                    count++;
                }
            }
        }
